...
def sinkhorn_knopp(A, epsilon=1e-9):
    # factor A, as explained in Algorithm 2
...
def generate_poisson_data(m=300, n=1000, r=10, log_mean=0, log_sigma=2):
    # the data is sampled according to the description in Appendix C.1
    # assert m < n
...
def find_alpha_beta(Y, plot=False):
    # adapt the model to the input data, as explained in section 5.1
...
def compare_uv(Y, alpha, beta):
    # compare factorization vector, as in figure 11
    var_Y = alpha * ((1 - beta) * Y + beta * Y ** 2)
    u_hat, v_hat = sinkhorn_knopp(var_Y)
...
def generate_NB_data(m=1000, n=2000, r=10, n_failures=3):
    # the data is sampled according to the description in Appendix E
...
def plot_eig_values_and_marchenko(Y, beta_range, singular_values=None, plot=False, name_to_save=None, after_BW=False):
    # plot the eigenvalues with the MP upper edge
...
def plot_density_and_marchenko(w, beta_range, rho, name_to_save=None, plot=False, after_bw=False):
    # plot the density of the eigenvalues vs. the MP density
...
def run_Biwhitening(Y=None, simulation_type=None, alpha=None, beta=None, name_to_save=None, plot=False):
    ...
    if alpha is not None and beta is not None:
        ...
        # estimate alpha and beta, as explained in section 5.1
    ...
    # plots before applying the algorithm
    w = plot_eig_values_and_marchenko(Y, beta_range, plot=plot,
    ...
    # main algorithm
    # factor with SK algorithm
    x, y = sinkhorn_knopp(var_Y)

    # estimate the rank, based on Algorithm 1
    beta_range, rho = marcenkoPastur(Y)
    ...
    # plots after applying the algorithm
...
def run_on_adata(adata, alpha=None, beta=None, plot=False, name_to_save=False):
    # run algorithm for input of type adata 
...
def run_simulation(simulation_type):
    # run the simulations from the paper
    print(f'run BiWhitening Simulation with {simulation_type} distribution')
    run_Biwhitening(simulation_type=simulation_type, plot=True)


if __name__ == '__main__':
    run_simulation(simulation_type=POISSON)
    run_simulation(simulation_type=NB)
    run_simulation(simulation_type=NB_FAILURES)
